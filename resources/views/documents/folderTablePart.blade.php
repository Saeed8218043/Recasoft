...
<div class="m-portlet panel-has-radius mb-4 custom-p-5">
    ...
    <div class="mb-2">
        <ul class="breadcrumb">
            <li><a href="{{ url('documents') }}/{{ $company_id }}">Home</a> </li>
            @foreach ($myuser['data'] as $data)
                @php
                    $folder = \App\Document::where('id', $data)->first();
                @endphp
                <li><a href="{{ url('documents') }}/{{ $company_id }}/{{ $folder->slug }}">{{ $folder->name }} </a>
                </li>
                @if ($folder->slug == $slug)
                @break;
            @endif
...
@php
    $user_folder = \App\Document::where('user_id', \Auth::user()->id)
        ->where('slug', $slug)
        ->first();
    $folder = \App\Document::where('slug', $slug)->first();
    $role = \App\CompanyMembers::where('user_id', $folder->user_id)->first();
@endphp
<div class="mb-1 mt-3 d-flex align-items-center flex-wrap">
    @if ($role == null)
        @if (isset($user_folder) && \Auth::user()->id == 1)
            <div>
                <button type="button" class="btn btn-primary " data-toggle="modal" data-target="#folder-modal">Create folder</button>
            </div>
            <button type="button" style="margin-left: 4px;" class="btn btn-primary " data-toggle="modal"
                data-target="#file-modal">Upload file</button>
        @endif
    @else
        @if ((isset($user_folder) && $role->role == 2) || $role->role == 1 || \Auth::user()->id == 1)
            <div>
                <button type="button" class="btn btn-primary " data-toggle="modal"
                    data-target="#folder-modal">Create folder</button>
            </div>
            <div>
                <button type="button" style="margin-left: 4px;" class="btn btn-primary " data-toggle="modal"
                    data-target="#file-modal">Upload file</button>
    ...
    @endif
    <div class="form-group m-form__group m-2">
    ...
    @if (isset($search) && $search != '')
        <button type="button" class="btn btn-primary ml-3" id="reset-filter">Reset Filter</button>
    @endif
</div>
            ...
            @if (isset($documents))
                @foreach ($documents as $row)
                    <tr>
                        ...
                        <td align="center">
                            @if ($row->type == 0)
                                <a id="{{ $row->id }}" class="iconHolder fig-40 sensor-icon-main bg-white shadow panel-has-radius c-border-1 mb-0 mr-2 relative"
                                    href="{{ url('documents') }}/{{ $company_id }}/{{ $row->slug }}"
                        ...
                        <td>
                            <a id="{{ $row->id }}"
                                ...
                                style="color:#212529;text-decoration:none;display:block;">{{ $row->name }}</a>
                        </td>
                        @php
                        ...
                        <td class="text-right">
                            ...
                            @if ($row->user_id == \Auth::user()->id || \Auth::user()->id == 1)
                                <span class="miniIcon">
                                    <i data-url="{{ route('documents.deletefolder', ['id' => $row->id]) }}"
                                        data-id="{{ $row->id }}" class="deletefolder la la-trash"></i>
                                </span>
                                <span class="miniIcon">
                                    <i data-dismiss="modal" class="editfolder la la-edit" data-toggle="modal"
                                        data-id="{{ $row->id }}" data-target="#modal-edit-folder"></i>
                                </span>
                            @else
                                @if ($role == 1 || $role == 2)
                                    @if (($row->user_id == \Auth::user()->id && $role == 2) || $role == 1 || \Auth::user()->id == 1)
                                        <span class="miniIcon">
                                            <i data-url="{{ route('documents.deletefolder', ['id' => $row->id]) }}"
                                                data-id="{{ $row->id }}"
                            ...
                            @endif
                            @if ($row->type == 1)
                                {{-- <a href="{{ route('documents.downloadFile', [$row->id]) }}"
                                    style=" color: black;">
                                    <span class="miniIcon">
                                ...
                                @if ($fileExtension === 'pdf')
                                    <a href="#"
                                        onclick='openModal("{{ asset("storage/app/public/$row->file") }}", "pdf")'
                                        style=" color: black;">
                                        <span class="miniIcon">
                                            <i data-dismiss="modal" class="la la-download"
                                                data-toggle="modal"></i>
                                        </span>
                                    </a>
                                @elseif (in_array($fileExtension, ['png', 'jpg', 'jpeg', 'gif']))
                                    <a href="#"
                                        onclick='openModal("{{ asset("storage/app/public/$row->file") }}", "image")'
                                        style=" color: black;">
                                        <span class="miniIcon">
                                            <i data-dismiss="modal" class="la la-download"
                                                data-toggle="modal"></i>
